...
/* ──────── BOTTOM SHEET ──────── */
const BottomSheet = ({ t, open, onClose, title, children }) => (
  <>
    <div onClick={onClose} style={{
      position:'absolute', inset:0, background:'rgba(10,10,10,0.55)',
      opacity:open?1:0, pointerEvents:open?'auto':'none',
      transition:'opacity .25s', zIndex:200,
    }}/>
    <div style={{
      ...
      <div style={{ display:'flex', justifyContent:'center', padding:'10px 0 4px' }}>
        <span style={{ width:38, height:4, background:t.borderSolid, borderRadius:r.pill }}/>
      </div>
      <div style={{ padding:'8px 20px 12px', display:'flex', alignItems:'center', justifyContent:'space-between',
        borderBottom:`1px solid ${t.border}` }}>
        <div style={{ ...T.button, fontSize:12, letterSpacing:1.6 }}>{title}</div>
        <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:t.text, padding:0, display:'flex' }}>
          <X size={18} strokeWidth={1.5}/>
        </button>
      </div>
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:'18px 20px 24px' }}>
        {children}
      </div>
    </div>
  </>
...
/* ──────── PHONE FRAME ──────── */
const PhoneFrame = ({ children, t, label, platform = 'ios' }) => {
  const isAndroid = platform === 'android';
  const outerRadius = isAndroid ? 32 : 38;
  const innerRadius = isAndroid ? 26 : 32;
  const statusH = isAndroid ? 26 : 28;
  const indicatorH = isAndroid ? 22 : 18;

  const StatusIos = () => (
    <div style={{
      height:statusH, padding:'6px 18px 0', display:'flex', justifyContent:'space-between', alignItems:'center',
      ...
      <div style={{ position:'absolute', left:'50%', top:8, transform:'translateX(-50%)',
        width:90, height:22, borderRadius:999, background:'#000' }}/>
  ...
  const StatusAndroid = () => (
    <div style={{
      height:statusH, padding:'4px 14px 0', display:'flex', justifyContent:'space-between', alignItems:'center',
      fontSize:11, fontWeight:600, color:t.text, position:'relative',
    }}>
      ...
      <span style={{ display:'flex', alignItems:'center', gap:6 }}>
        <span style={{ display:'inline-flex', gap:1, alignItems:'flex-end' }}>
          {[3,5,7,9].map(h => <span key={h} style={{ width:2.5, height:h, background:t.text, borderRadius:0.5 }}/>)}
        </span>
        ...
        <span style={{
          ...
          padding:'1px 5px', border:`1px solid ${t.text}`, borderRadius:3, fontSize:8.5, fontWeight:700,
        }}>
          <span style={{ width:6, height:5, background:t.text, borderRadius:1 }}/>
          82
        </span>
      </span>
    </div>
  ...
  const NavIos = () => (
    <div style={{ position:'absolute', bottom:0, left:0, right:0, height:indicatorH,
  ...
  const NavAndroid = () => (
    <div style={{ position:'absolute', bottom:0, left:0, right:0, height:indicatorH,
      display:'flex', alignItems:'center', justifyContent:'center', background:t.bg, gap:2 }}>
      <div style={{ width:74, height:3.5, background:t.text, opacity:0.85, borderRadius:999 }}/>
    </div>
  );
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14 }}>
      <div style={{
        width:320, height:678, borderRadius:outerRadius, padding:7, background:'#0A0A0A',
        boxShadow:'0 30px 60px -20px rgba(0,0,0,0.35), 0 0 0 1.5px rgba(255,255,255,0.04) inset',
        position:'relative',
      }}>
        <div style={{
          width:'100%', height:'100%', borderRadius:innerRadius, background:t.bg,
          overflow:'hidden', position:'relative', fontFamily:'Inter,-apple-system,sans-serif', color:t.text,
        }}>
          {isAndroid ? <StatusAndroid/> : <StatusIos/>}
          <div style={{ height:`calc(100% - ${statusH}px - ${indicatorH}px)`, display:'flex', flexDirection:'column', position:'relative' }}>
            {children}
          </div>
          {isAndroid ? <NavAndroid/> : <NavIos/>}
        </div>
      </div>
...
const Header = ({ t, go, title, back='home', right }) => (
  <div style={{ padding:'14px 20px', display:'flex', alignItems:'center', gap:14,
    ...
    <button onClick={() => go(back)} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, display:'flex' }}>
  ...
  <>
    ...
    <div style={{
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
        ...
        <div style={{ height:1, background:t.border, margin:'8px 0' }}/>
        <div style={{ padding:'4px 0' }}>
          {[{ l:'Find a store', i:Store },{ l:'Help & contact', i:MessageCircle },{ l:'Settings', i:Settings }].map(row => (
            <button key={row.l} onClick={close} style={{
              width:'100%', padding:'12px 20px', background:'transparent', border:'none', textAlign:'left',
              display:'flex', alignItems:'center', gap:14, ...T.bodySm, fontWeight:500, color:t.text, fontFamily:'inherit', cursor:'pointer',
            }}>
...
/* ──────── LOGIN ──────── */
const UnderlineField = ({ t, label, value, onChange, type, placeholder, trailing, autoComplete, inputMode }) => (
...
const LoginScreen = ({ t, go }) => {
  ...
  const [email, setEmail] = useState('alex@vision.com');
  const [password, setPassword] = useState('••••••••••');
  return (
    <div style={{ flex:1, padding:'24px 22px', display:'flex', flexDirection:'column', background:t.bg, overflowY:'auto', WebkitOverflowScrolling:'touch', minHeight:0 }}>
      <button onClick={() => go('splash')} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, alignSelf:'flex-start', display:'flex' }}>
      ...
      <div style={{ marginTop:36 }}>
        <UnderlineField t={t} label="EMAIL OR MOBILE" value={email} onChange={(e) => setEmail(e.target.value)}
          autoComplete="email" inputMode="email" placeholder="you@vision.com"/>
        <UnderlineField t={t} label="PASSWORD" value={password} onChange={(e) => setPassword(e.target.value)}
        ...
        <div style={{ textAlign:'right', marginTop:16 }}>
          <button onClick={() => go('login')} style={{ ...T.buttonSm, color:t.textDim, borderBottom:`1px solid ${t.textDim}`, paddingBottom:1, cursor:'pointer', background:'none', border:'none', fontFamily:'inherit' }}>Forgot password</button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {[{ i:Apple, l:'Continue with Apple' },{ i:Globe, l:'Continue with Google' }].map(s => (
          <button key={s.l} onClick={() => go('home')} style={{
      ...
      <div style={{ marginTop:'auto', textAlign:'center', ...T.caption, color:t.textDim, paddingTop:28 }}>
        New to Vision?{' '}
        <button onClick={() => go('home')} style={{ color:t.text, fontWeight:600, borderBottom:`1px solid ${t.text}`, cursor:'pointer', paddingBottom:1, background:'none', border:'none', fontFamily:'inherit', fontSize:'inherit' }}>Become a member</button>
      </div>
    </div>
  );
};
...
const HomeScreen = ({ t, go, toggleWish, wishlist, cartCount, openDrawer, tier, spend }) => {
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', paddingBottom:64, background:t.bg }}>
      <div style={{ padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center',
        position:'sticky', top:0, background:t.bg, zIndex:10, borderBottom:`1px solid ${t.border}` }}>
        <button onClick={openDrawer} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, display:'flex' }}>
          <Menu size={19} strokeWidth={1.5}/>
        </button>
        <div style={{ ...T.wordmark }}>VISION<span style={{ color:t.primary }}>.</span></div>
        <div style={{ display:'flex', gap:14 }}>
          <button onClick={() => go('search')} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, display:'flex' }}>
            <Search size={17} strokeWidth={1.5}/>
          </button>
          <button onClick={() => go('cart')} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, position:'relative', display:'flex' }}>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      <div style={{ position:'sticky', top:0, zIndex:10, background:t.bg }}>
        <div style={{ padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <button onClick={() => go('home')} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, display:'flex' }}>
            <ArrowLeft size={18} strokeWidth={1.5}/>
          </button>
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      <div style={{ position:'sticky', top:0, zIndex:10, background:t.bg, borderBottom:`1px solid ${t.border}` }}>
        <div style={{ padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <button onClick={() => sharePage('Story N°04 — The art of restraint')} style={{ background:'none', border:'none', padding:0, cursor:'pointer', color:t.text, display:'flex' }}>
...
const SearchScreen = ({ t, go, toggleWish, wishlist }) => {
  ...
  const [sortIdx, setSortIdx] = useState(0);
  const [saleOnly, setSaleOnly] = useState(false);
  const baseList = q ? PRODUCTS.filter(p => p.name.toLowerCase().includes(q.toLowerCase()) || p.brand.toLowerCase().includes(q.toLowerCase())) : PRODUCTS;
  const filtered = saleOnly ? baseList.filter(p => p.sale) : baseList;
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      ...
      {!q ? (
        <div style={{ flex:1, overflowY:'auto', minHeight:0, padding:'20px 20px 80px' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:14 }}>
              {['Vision Studio','Spring ’26','Wide-leg','Cashmere','SALE'].map(s => (
        ...
        <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', minHeight:0, paddingBottom:80 }}>
          ...
          {list.length === 0 ? (
            <div style={{ padding:'60px 30px', textAlign:'center', ...T.bodySm, color:t.textDim }}>
              No products match.
            </div>
          ) : (
...
/* ──────── PRODUCT ──────── */
const COLOR_NAMES = ['Stone', 'Sand', 'Onyx'];
const SIZE_GUIDE = [
...
const ProductScreen = ({ t, go, goBack, product, addToCart, toggleWish, wished, tier }) => {
  ...
  const swatches = [p.bg[0], p.bg[2], '#1F1F1F'];

  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', minHeight:0, paddingBottom:80 }}>
        <div style={{ position:'relative' }}>
          ...
          <div style={{ position:'absolute', top:16, left:0, right:0, padding:'0 20px',
            ...
            <div style={{ display:'flex', gap:6 }}>
              <button onClick={() => sharePage(`${p.brand} — ${p.name}`)} style={{
                width:38, height:38, borderRadius:r.pill, border:'none',
                background:'rgba(255,255,255,0.85)', backdropFilter:'blur(10px)', cursor:'pointer',
                display:'flex', alignItems:'center', justifyContent:'center', color:'#1A1815',
        ...
        <div style={{ padding:'24px 20px' }}>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12 }}>
              <Eyebrow t={t}>Colour — {COLOR_NAMES[colorIdx]}</Eyebrow>
            ...
            <div style={{ display:'flex', gap:10 }}>
              {swatches.map((c,i) => (
                <button
                  ...
                  aria-label={`Select colour ${COLOR_NAMES[i]}`}
                  aria-pressed={colorIdx===i}
                  style={{
                    width:44, height:56, background:c, borderRadius:r.sm, cursor:'pointer',
                    outline: colorIdx===i?`1.5px solid ${t.text}`:'none', outlineOffset:3,
                    border:'none', padding:0, transition:'outline .15s',
                  }}
                />
              ))}
            </div>
          ...
          <div style={{ marginTop:28 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12 }}>
              ...
              <button onClick={() => setSizeGuideOpen(true)} style={{ ...T.buttonSm, color:t.textDim, borderBottom:`1px solid ${t.textDim}`, paddingBottom:1, cursor:'pointer', background:'none', border:'none', fontFamily:'inherit' }}>Size guide</button>
            </div>
      ...
      <div style={{ position:'absolute', bottom:0, left:0, right:0,
        ...
        <button onClick={() => { if (!size) { setSizeGuideOpen(true); return; } addToCart(p); go('cart'); }} style={{
          flex:1, background:t.text, color:t.bg, border:'none', cursor:'pointer',
          ...T.button, fontFamily:'inherit', borderRadius:r.md,
          display:'flex', alignItems:'center', justifyContent:'center', gap:8,
        }}>{size ? `Add to bag — $${memberDiscount?finalPrice:p.price}` : 'Select size'}</button>
      </div>

      <BottomSheet t={t} open={sizeGuideOpen} onClose={() => setSizeGuideOpen(false)} title="Size guide">
        <div style={{ ...T.bodySm, color:t.textDim, marginBottom:14 }}>
          Measurements in cm. Pick the column closest to your body — sizes are cut for an easy, modern fit.
        </div>
        <div style={{ border:`1px solid ${t.border}`, borderRadius:r.md, overflow:'hidden' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', background:t.bg2,
            ...T.eyebrowS, padding:'12px 14px', color:t.textDim }}>
            <span>SIZE</span><span>CHEST</span><span>WAIST</span><span>HIPS</span>
          </div>
          {SIZE_GUIDE.map((row, i) => {
            const selected = size === row.size;
            const disabled = row.size === 'XL';
            return (
              <button
                key={row.size}
                disabled={disabled}
                onClick={() => { setSize(row.size); setSizeGuideOpen(false); }}
                style={{
                  ...
                  color: disabled ? t.textMute : t.text, textAlign:'left',
                  ...T.bodySm, fontWeight:500, fontFamily:'inherit',
                  border:'none', borderTopColor:t.border, cursor:disabled?'not-allowed':'pointer',
                  textDecoration:disabled?'line-through':'none',
                }}
              >
                <span style={{ fontWeight:700 }}>{row.size}</span>
                <span>{row.chest}</span>
                <span>{row.waist}</span>
                <span>{row.hips}</span>
              </button>
...
/* ──────── CART ──────── */
const PROMOS = [
  { code:'SS26',   name:'New season',     sub:'15% off SS’26 outerwear',   discount:0.15 },
  { code:'WELCOME10', name:'Welcome',     sub:'$10 off your first order',      discount:10 },
  { code:'STUDIO5',name:'Studio members', sub:'5% off Vision Studio essentials', discount:0.05 },
];
...
const CartScreen = ({ t, go, cart, setCart, tier }) => {
  ...
  const promoOff = !promo ? 0 : (promo.discount < 1 ? subtotal * promo.discount : Math.min(promo.discount, subtotal));
  const taxable = Math.max(0, subtotal - promoOff);
  const tax = taxable * 0.08;
  const total = taxable + tax;
  const tierIdx = TIERS.findIndex(x => x.id === tier.id);
  const earnPercent = [0,5,10,15][tierIdx];
  const earned = Math.round(taxable * earnPercent / 100);
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', minHeight:0, paddingBottom:200 }}>
        {cart.map(item => (
        ...
        <button onClick={() => setOffersOpen(true)} style={{
          margin:'0 20px 20px', padding:'14px 16px', border:`1px solid ${t.borderStrong}`, borderRadius:r.md,
          display:'flex', alignItems:'center', gap:12, width:'calc(100% - 40px)', background:'transparent',
          color:t.text, cursor:'pointer', fontFamily:'inherit', textAlign:'left',
        }}>
          <Tag size={14} strokeWidth={1.5}/>
          <div style={{ flex:1 }}>
            <div style={{ ...T.buttonSm, fontSize:11, letterSpacing:1.4 }}>
              {promo ? `${promo.code} applied` : 'Add code or member offer'}
            </div>
            <div style={{ ...T.caption, color:promo?t.success:t.textDim, marginTop:2 }}>
              {promo ? `−$${promoOff.toFixed(2)} off` : `${PROMOS.length} offers available`}
            </div>
          </div>
          {promo
            ? <X size={14} strokeWidth={1.5} onClick={(e) => { e.stopPropagation(); setPromo(null); }}/>
        ...
        <div style={{ padding:'8px 20px 20px' }}>
          ...
          <div style={{ marginTop:14 }}>
            {[
              ...
              ...(promo ? [[`Offer · ${promo.code}`, `−$${promoOff.toFixed(2)}`]] : []),
              ...
              <div key={l} style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', ...T.bodySm }}>
                ...
                <span style={{ color:l.toString().startsWith('Offer')?t.success:t.text, fontWeight:500 }}>{v}</span>
              </div>
            ))}
      ...
      <BottomSheet t={t} open={offersOpen} onClose={() => setOffersOpen(false)} title="Offers & codes">
        <div style={{ ...T.bodySm, color:t.textDim, marginBottom:14 }}>
          One offer can be applied per order. Member rewards are added automatically at checkout.
        </div>
        {PROMOS.map(o => {
          const active = promo?.code === o.code;
          return (
            <button
              key={o.code}
              onClick={() => { setPromo(active ? null : o); setOffersOpen(false); }}
              style={{
                width:'100%', textAlign:'left', padding:'14px 16px', marginBottom:8, borderRadius:r.md,
                border: active?`1.5px solid ${t.text}`:`1px solid ${t.borderSolid}`,
            ...
            >
              ...
              <div style={{ flex:1 }}>
                <div style={{ ...T.bodySm, fontWeight:600 }}>{o.name} <span style={{ color:t.textDim, fontWeight:400 }}>· {o.code}</span></div>
                <div style={{ ...T.caption, color:t.textDim, marginTop:2 }}>{o.sub}</div>
              </div>
...
const CheckoutScreen = ({ t, go, cart }) => {
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      ...
      <Steps t={t} active={0}/>
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', minHeight:0, paddingBottom:110 }}>
...
const OrdersScreen = ({ t, go }) => {
  ...
  const orders = [
    { id:'VSN-72491', date:'Today · 9:42', status:'Out for delivery', step:2, items:3, total:547.89, color:t.primary, group:'Active' },
    { id:'VSN-72380', date:'Yesterday',     status:'Delivered',        step:3, items:1, total:78.00,  color:t.success, group:'Delivered' },
    { id:'VSN-71998', date:'12 May',         status:'Delivered',        step:3, items:2, total:198.50, color:t.success, group:'Delivered' },
    { id:'VSN-71755', date:'4 May',          status:'Cancelled',        step:0, items:1, total:64.00,  color:t.textDim, group:'Cancelled' },
  ];
  const visible = tab === 'All' ? orders : orders.filter(o => o.group === tab);
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', minHeight:0, paddingBottom:30 }}>
        {visible.length === 0 && (
          <div style={{ padding:'60px 30px', textAlign:'center', ...T.bodySm, color:t.textDim }}>
        ...
        {visible.map(o => (
          <button key={o.id} onClick={() => go('orders')} style={{
            display:'block', width:'100%', textAlign:'left', padding:20,
            borderBottom:`1px solid ${t.border}`, background:'transparent', border:'none',
            borderBottomColor:t.border, color:t.text, cursor:'pointer', fontFamily:'inherit',
          }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop: o.step>0&&o.step<3?0:14 }}>
              ...
              <span style={{ ...T.buttonSm, borderBottom:`1px solid ${t.text}`, paddingBottom:1 }}>View</span>
            </div>
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
      ...
      {list.length === 0 ? (
        ...
        <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', minHeight:0, padding:'20px 20px 80px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
...
const ProfileScreen = ({ t, go, theme, setTheme, tier, spend }) => (
  <div style={{ flex:1, display:'flex', flexDirection:'column', background:t.bg }}>
    ...
    <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', overscrollBehavior:'contain', minHeight:0, paddingBottom:80 }}>
      ...
      <ProfileSection t={t} title="Shopping" rows={[
        ...
        { i:MapPin, l:'Addresses', h:'2 saved' },
        { i:CreditCard, l:'Payment methods' },
      ]}/>
      <ProfileSection t={t} title="More" rows={[
    ...
    <div style={{ minHeight:'100vh', background: theme==='light'?'#EDE8DD':'#080705',
      ...
      <style>{`
        ...
        * { box-sizing:border-box; -webkit-font-smoothing:antialiased; }
        body { margin:0; font-family:Inter,sans-serif; -webkit-tap-highlight-color:transparent; }
        .no-scrollbar::-webkit-scrollbar { display:none; }
        .no-scrollbar { -ms-overflow-style:none; scrollbar-width:none; }
        button, input { font-family:inherit; }
        button { -webkit-tap-highlight-color:transparent; touch-action:manipulation; }
        [data-scroll="y"] { overflow-y:auto; -webkit-overflow-scrolling:touch; overscroll-behavior:contain; }
        [data-scroll="x"] { overflow-x:auto; -webkit-overflow-scrolling:touch; overscroll-behavior-x:contain; touch-action:pan-x; }
      `}</style>
      ...
      <div style={{ maxWidth:1280, margin:'0 auto 36px',
        ...
        <div>
          ...
          <div style={{ ...T.eyebrowS, color:t.textDim, marginTop:6 }}>{platform==='android'?'ANDROID':'iOS'} PROTOTYPE — SS&rsquo;26 · V4</div>
        </div>
          ...
          <div style={{ display:'flex', padding:3, gap:2,
            ...
            {['android','ios'].map(p => (
              <button key={p} onClick={() => setPlatform(p)} style={{
                ...
                background: platform===p?t.text:'transparent', color: platform===p?t.bg:t.text, ...T.buttonSm,
                display:'flex', alignItems:'center', gap:6,
              }}>
                {p==='android' ? <Smartphone size={12} strokeWidth={1.5}/> : <Apple size={12} strokeWidth={1.5}/>}
      ...
      {view === 'interactive' ? (
        <div style={{ display:'flex', justifyContent:'center' }}>
          <PhoneFrame t={t} platform={platform}>
            {renderScreen()}
        ...
        <div style={{ maxWidth:1280, margin:'0 auto',
          ...
          {showcase.map(s => (
            <PhoneFrame key={s.id} t={t} platform={platform} label={s.label}>
